...
from ._aligner import Aligner, PhonemeAlignment, WordAlignment
from .config import PiperConfig, SynthesisConfig
from .const import BOS, EOS, PAD
from .phonemize_espeak import ESPEAK_DATA_DIR
...
class PiperVoice:
    ...
    def aligner(self) -> Aligner:
        """Aligner for this voice."""
        if not hasattr(self, "_aligner"):
            self._aligner = Aligner(self)
    ...
    def phonemize(self, text: str) -> list[list[str]]:
        ...
        """
        return self.aligner.get_phonemes_from_sentences(
            self.aligner.phonemize_text(text)
        )
    ...
    def synthesize(
        ...
        sentence_phonemes_aligned = self.aligner.phonemize_text(text)
        sentence_phonemes = self.aligner.get_phonemes_from_sentences(
            sentence_phonemes_aligned
        )
        ...
        for i, phonemes in enumerate(sentence_phonemes):
            ...
            phoneme_ids = self.aligner.phonemes_to_ids(phonemes)
            ...
            word_alignments = (
                ...
                if phoneme_alignments is None
                    ...
                    list(
                        self.aligner.align(
                            phoneme_alignments, sentence_phonemes_aligned[i]
                        )
                    )
                )
            )
